...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Plg8: ",verde)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        Validar("timerAccion8", "PrePuertAbiert")
        Validar("scanCycle8", "PrePuertAbiert")
        StartTimer(Tiempo1)
        return true
    end
    if SensorPuerta.value == false then
        if _storage.get_number("accion8") == ACCION_PABIERTA or _storage.get_number("accion8") == ACCION_LIBRE then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                ...
                if _storage.get_number("accion8") == ACCION_SCAN then
                    ...
                    Validar("timerAccion8", "Ocupado")
                    Validar("scanCycle8", "Ocupado")
                    Registro("se paso al la accion 8 ocupado por disparo del sensor de movimiento", verde)
                    return
                end
...
function EventoActuadores(Actuador)
    ...
    for _, id in ipairs(Actuador) do
        ...
        if actuador_id.value == true then
            ...
            if (tiempoRestante8 >= 20 and (data.scancycle == "scan_One" or data.scancycle == "scan_Two" or data.scancycle == "scan_Libre")) then
                Registro("Puerta Abierta Scan Libre")
                if _storage.get_number("accion8") == ACCION_CICLO_APAGADO and PreOffAireDoor == "si" then
                    Registro("on actuadores", amarillo)
                    Rutina_Encendido()
                end
                CancelTimer()
...
function FunPuertaAbierta()
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Validar("timerAccion8", "PuertaAbierta")
    Validar("scanCycle8", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then        
        ...
        CancelTimer()
        ActualizarAccion(ACCION_LIBRE)
        Validar("statusText8", "Libre")
        Validar("timerAccion8", "Libre")
        setItemValue(EstadoTextoId,"Libre")
        StartTimer(TiempoLibre)

        -- rutina Comentada // Investigar con la org
        if _storage.get_string("Modo8") ~= MODO_APAGADO then
            if ModoOffPuertaAbierta == "si" then
                Rutina_Apagado()
            end
        end
...
function StartTimeralways()
    ...
    if data.TimerID ~= "" and status then
        ...
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/rooms/room8",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/rooms/room8",
            { arg_name = "timer" })
        _storage.set_string("TimerID8", tostring(timerID))
        return true
    end
...
function Main()
    ...
    Registro("::: Manejos Internos ::: Plg8")
    ...
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde) --"TimerAccion8: " .. _json.encode(data.timeraccion)..
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo8", tostring(modeButton.value)) --Actualiza estado del Plg con respecto al cambio de estado en web
    Validar("scanCycle8", "scan_One")
    Validar("timerAccion8", "Scan")

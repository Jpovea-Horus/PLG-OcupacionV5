...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Plg4: ",verde)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        Validar("timerAccion4", "PrePuertAbiert")
        Validar("scanCycle4", "PrePuertAbiert")
        StartTimer(Tiempo1)
        return true
    end
    if SensorPuerta.value == false then
        if _storage.get_number("accion4") == ACCION_PABIERTA or _storage.get_number("accion4") == ACCION_LIBRE then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                ...
                if _storage.get_number("accion4") == ACCION_SCAN then
                    ...
                    Validar("timerAccion4", "Ocupado")
                    Validar("scanCycle4", "Ocupado")
                    Registro("se paso al la accion 4 ocupado por disparo del sensor de movimiento", verde)
                    return
                end
...
function EventoActuadores(Actuador)
    ...
    for _, id in ipairs(Actuador) do
        ...
        if actuador_id.value == true then
            ...
            if (tiempoRestante4 >= 20 and (data.scancycle == "scan_One" or data.scancycle == "scan_Two" or data.scancycle == "scan_Libre")) then
                Registro("Puerta Abierta Scan Libre")
                if _storage.get_number("accion4") == ACCION_CICLO_APAGADO and PreOffAireDoor == "si" then
                    Registro("on actuadores", amarillo)
                    Rutina_Encendido()
                end
                CancelTimer()
...
function FunPuertaAbierta()
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Validar("timerAccion4", "PuertaAbierta")
    Validar("scanCycle4", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then        
        ...
        CancelTimer()
        ActualizarAccion(ACCION_LIBRE)
        Validar("statusText4", "Libre")
        Validar("timerAccion4", "Libre")
        setItemValue(EstadoTextoId,"Libre")
        StartTimer(TiempoLibre)

        -- rutina Comentada // Investigar con la org
        if _storage.get_string("Modo4") ~= MODO_APAGADO then
            if ModoOffPuertaAbierta == "si" then
                Rutina_Apagado()
            end
        end
...
function StartTimeralways()
    ...
    if data.TimerID ~= "" and status then
        ...
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/rooms/room4",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/rooms/room4",
            { arg_name = "timer" })
        _storage.set_string("TimerID4", tostring(timerID))
        return true
    end
...
function Main()
    ...
    Registro("::: Manejos Internos ::: Plg4")
    ...
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde) --"TimerAccion4: " .. _json.encode(data.timeraccion)..
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo4", tostring(modeButton.value)) --Actualiza estado del Plg con respecto al cambio de estado en web
    Validar("scanCycle4", "scan_One")
    Validar("timerAccion4", "Scan")

...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Plg6: ",verde)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        Validar("timerAccion6", "PrePuertAbiert")
        Validar("scanCycle6", "PrePuertAbiert")
        StartTimer(Tiempo1)
        return true
    end
    if SensorPuerta.value == false then
        if _storage.get_number("accion6") == ACCION_PABIERTA or _storage.get_number("accion6") == ACCION_LIBRE then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                ...
                if _storage.get_number("accion6") == ACCION_SCAN then
                    ...
                    Validar("timerAccion6", "Ocupado")
                    Validar("scanCycle6", "Ocupado")
                    Registro("se paso al la accion 6 ocupado por disparo del sensor de movimiento", verde)
                    return
                end
...
function EventoActuadores(Actuador)
    ...
    for _, id in ipairs(Actuador) do
        ...
        if actuador_id.value == true then
            ...
            if (tiempoRestante6 >= 20 and (data.scancycle == "scan_One" or data.scancycle == "scan_Two" or data.scancycle == "scan_Libre")) then
                Registro("Puerta Abierta Scan Libre")
                if _storage.get_number("accion6") == ACCION_CICLO_APAGADO and PreOffAireDoor == "si" then
                    Registro("on actuadores", amarillo)
                    Rutina_Encendido()
                end
                CancelTimer()
...
function FunPuertaAbierta()
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Validar("timerAccion6", "PuertaAbierta")
    Validar("scanCycle6", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then        
        ...
        CancelTimer()
        ActualizarAccion(ACCION_LIBRE)
        Validar("statusText6", "Libre")
        Validar("timerAccion6", "Libre")
        setItemValue(EstadoTextoId,"Libre")
        StartTimer(TiempoLibre)

        -- rutina Comentada // Investigar con la org
        if _storage.get_string("Modo6") ~= MODO_APAGADO then
            if ModoOffPuertaAbierta == "si" then
                Rutina_Apagado()
            end
        end
...
function StartTimeralways()
    ...
    if data.TimerID ~= "" and status then
        ...
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/rooms/room6",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/rooms/room6",
            { arg_name = "timer" })
        _storage.set_string("TimerID6", tostring(timerID))
        return true
    end
...
function Main()
    ...
    Registro("::: Manejos Internos ::: Plg6")
    ...
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde) --"TimerAccion6: " .. _json.encode(data.timeraccion)..
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo6", tostring(modeButton.value)) --Actualiza estado del Plg con respecto al cambio de estado en web
    Validar("scanCycle6", "scan_One")
    Validar("timerAccion6", "Scan")

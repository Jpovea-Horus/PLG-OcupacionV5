...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Plg2: ",verde)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        Validar("timerAccion2", "PrePuertAbiert")
        Validar("scanCycle2", "PrePuertAbiert")
        StartTimer(Tiempo1)
        return true
    end
    if SensorPuerta.value == false then
        if _storage.get_number("accion2") == ACCION_PABIERTA or _storage.get_number("accion2") == ACCION_LIBRE then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                ...
                if _storage.get_number("accion2") == ACCION_SCAN then
                    ...
                    Validar("timerAccion2", "Ocupado")
                    Validar("scanCycle2", "Ocupado")
                    Registro("se paso al la accion 2 ocupado por disparo del sensor de movimiento", verde)
                    return
                end
...
function EventoActuadores(Actuador)
    ...
    for _, id in ipairs(Actuador) do
        ...
        if actuador_id.value == true then
            ...
            if (tiempoRestante2 >= 20 and (data.scancycle == "scan_One" or data.scancycle == "scan_Two" or data.scancycle == "scan_Libre")) then
                Registro("Puerta Abierta Scan Libre")
                if _storage.get_number("accion2") == ACCION_CICLO_APAGADO and PreOffAireDoor == "si" then
                    Registro("on actuadores", amarillo)
                    Rutina_Encendido()
                end
                CancelTimer()
...
function FunPuertaAbierta()
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Validar("timerAccion2", "PuertaAbierta")
    Validar("scanCycle2", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then        
        ...
        CancelTimer()
        ActualizarAccion(ACCION_LIBRE)
        Validar("statusText2", "Libre")
        Validar("timerAccion2", "Libre")
        setItemValue(EstadoTextoId,"Libre")
        StartTimer(TiempoLibre)

        -- rutina Comentada // Investigar con la org
        if _storage.get_string("Modo2") ~= MODO_APAGADO then
            if ModoOffPuertaAbierta == "si" then
                Rutina_Apagado()
            end
        end
...
function StartTimeralways()
    ...
    if data.TimerID ~= "" and status then
        ...
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/rooms/room2",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/rooms/room2",
            { arg_name = "timer" })
        _storage.set_string("TimerID2", tostring(timerID))
        return true
    end
...
function Main()
    ...
    Registro("::: Manejos Internos ::: Plg2")
    ...
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde) --"TimerAccion2: " .. _json.encode(data.timeraccion)..
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo2", tostring(modeButton.value)) --Actualiza estado del Plg con respecto al cambio de estado en web
    Validar("scanCycle2", "scan_One")
    Validar("timerAccion2", "Scan")

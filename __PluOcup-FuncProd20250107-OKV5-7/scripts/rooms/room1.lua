...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Plg1: ",verde)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        Validar("timerAccion1", "PrePuertAbiert")
        Validar("scanCycle1", "PrePuertAbiert")
        StartTimer(Tiempo1)
        return true
    end
    if SensorPuerta.value == false then
        if _storage.get_number("accion1") == ACCION_PABIERTA or _storage.get_number("accion1") == ACCION_LIBRE then
...
function EventoActuadores(Actuador)
    ...
    for _, id in ipairs(Actuador) do
        ...
        if actuador_id.value == true then
            ...
            if (tiempoRestante1 >= 20 and (data.scancycle == "scan_One" or data.scancycle == "scan_Two" or data.scancycle == "scan_Libre")) then
                Registro("Puerta Abierta Scan Libre")
                if _storage.get_number("accion1") == ACCION_CICLO_APAGADO and PreOffAireDoor == "si" then
                    Registro("on actuadores", amarillo)
                    Rutina_Encendido()
                end
                CancelTimer()
...
function FunPuertaAbierta()
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Validar("timerAccion1", "PuertaAbierta")
    Validar("scanCycle1", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then        
        ...
        CancelTimer()
        ActualizarAccion(ACCION_LIBRE)
        Validar("statusText1", "Libre")
        Validar("timerAccion1", "Libre")
        setItemValue(EstadoTextoId,"Libre")
        StartTimer(TiempoLibre)

        -- rutina Comentada // Investigar con la org
        if _storage.get_string("Modo1") ~= MODO_APAGADO then
            if ModoOffPuertaAbierta == "si" then
                Rutina_Apagado()
            end
        end
...
function StartTimeralways()
    ...
    if data.TimerID ~= "" and status then
        ...
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/rooms/room1",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/rooms/room1",
            { arg_name = "timer" })
        _storage.set_string("TimerID1", tostring(timerID))
        return true
    end
...
function Main()
    ...
    Registro("::: Manejos Internos ::: Plg1")
    ...
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde) --"TimerAccion1: " .. _json.encode(data.timeraccion)..
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo1", tostring(modeButton.value)) --Actualiza estado del Plg con respecto al cambio de estado en web
    Validar("scanCycle1", "scan_One")
    Validar("timerAccion1", "Scan")
...
if sensor_puerta ~= nil and params.event == "item_updated" then
    if params._id == sensor_puerta then
        EventoPuerta()
    end
end
